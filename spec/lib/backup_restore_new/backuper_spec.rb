...
describe BackupRestoreNew::Backuper do
  ...
  def expect_db_dump_added_to_tar(tar_writer)
    ...
    tar_writer.expects(:add_file_from_stream).with(has_entry(name: "dump.sql.gz")).yields(output_stream).once
  end
  ...
  def expect_uploads_added_to_tar(tar_writer)
    ...
    tar_writer.expects(:add_file_from_stream).with(has_entry(name: "uploads.tar.gz")).yields(output_stream).once
  end
  ...
  def expect_optimized_images_added_to_tar(tar_writer)
    ...
    tar_writer.expects(:add_file_from_stream).with(has_entry(name: "optimized-images.tar.gz")).yields(output_stream).once
  end
  ...
  def expect_metadata_added_to_tar(tar_writer)
    ...
    tar_writer.expects(:add_file_from_stream).with(has_entry(name: "meta.json")).yields(output_stream).once
  end
