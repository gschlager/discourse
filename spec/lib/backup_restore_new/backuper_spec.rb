...
describe BackupRestoreNew::Backuper do
  ...
  def expect_db_dump_added_to_tar(tar_writer)
    db_dump_stream = mock("db_dump_output_stream")
    BackupRestoreNew::Backup::DatabaseDumper.any_instance.expects(:dump_schema).with(db_dump_stream).once
    tar_writer.expects(:add_file_from_stream).with(has_entry(name: "dump.sql.gz")).yields(db_dump_stream).once
  end

  def expect_uploads_added_to_tar(tar_writer)
    uploads_stream = mock("uploads_stream")
    BackupRestoreNew::Backup::UploadBackuper.any_instance.expects(:compress_uploads).with(uploads_stream).once
    tar_writer.expects(:add_file_from_stream).with(has_entry(name: "uploads.tar.gz")).yields(uploads_stream).once
  end
