...
module Migrations
  module Tooling
    module Coverage
      ...
      # column. Analyses every discovered converter, so an unverifiable call
      # site (a `**` splat or non-literal keyword) fails loudly even in a
      # non-reference converter. Prints its findings and returns whether the
      # check passed.
      class ReferenceCheck
        ...
        def analyze(converter_name)
          ConverterAnalyzer.new(Migrations::Converters.path_of(converter_name)).written_columns
        end
