...
module Migrations
  module Converters
    ...
    # natively. With no sink, the markup is just converted, nothing deferred.
    #
    # Markbridge does not parse Markdown, so a Discourse -> Discourse migration
    # (whose `raw` is already Markdown) does not use this; it is for forums whose
    # post bodies are in one of the formats above.
    #
