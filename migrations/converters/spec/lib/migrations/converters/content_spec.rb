...
RSpec.describe Migrations::Converters::Content do
  describe "#to_markdown without an embed sink" do
    subject(:content) { described_class.new(format: :bbcode) }

    it "converts BBCode to Discourse Markdown" do
      expect(content.to_markdown("[b]hi[/b] and [url=https://example.com]a link[/url]")).to eq(
  ...
  describe "#to_markdown deferring embeds into an EmbedBuffer" do
    ...
    it "renders an unattributed quote natively (nothing to remap)" do
      raw = content.to_markdown("[quote]just text[/quote]", on_embed: buffer)
    ...
    it "leaves links alone by default" do
      raw = content.to_markdown("see [url=https://example.com]here[/url]", on_embed: buffer)

      expect(buffer.links).to be_empty
      expect(raw).to eq("see [here](https://example.com)")
    end

    # The contract: every token in the converted raw maps to exactly one recorded
    # linkage descriptor.
